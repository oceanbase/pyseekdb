...
def _make_onnx() -> OnnxEmbeddingFunction:
...
def test_init_validates_hf_model_id() -> None:
...
def test_init_validates_dimension() -> None:
...
def test_init_validates_preferred_providers() -> None:
...
def test_call_empty_returns_empty() -> None:
    ef = _make_onnx()
    assert ef([]) == []


def test_call_generates_embeddings() -> None:
