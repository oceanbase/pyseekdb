...
class TestDefaultEmbeddingFunction:
    ...
    def test_default_embedding_function(self, db_client):
        ...
        results = collection.get(
            ids=test_ids[0], include=["documents", "metadatas", "embeddings"]
        )
        ...
        if results.get("embeddings") and results["embeddings"][0] is not None:
            ...
            print(
                f"   Verified: document and embedding (dim={len(results['embeddings'][0])}) stored correctly"
            )
        else:
            print(
                f"   Verified: document stored correctly (embedding not included in get results)"
            )
    ...
    def test_dimension_of(self):
        ...
        assert dim == default_ef.dimension, (
            "dimension_of should return the same as .dimension property"
        )
        ...
        with pytest.raises(
            ValueError, match="Embedding function returned empty result"
        ):
